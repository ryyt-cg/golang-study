...
### Pros
* LocalStack accelerates your dev and test feedback loop. It provides you with a fully functional local AWS cloud
  stack that gives immediate feedback on your changes without waiting on cloud resources. This saves you time and
  resources.

### Cons
* Fact that Localstack is not 100% mirror AWS Service. You might run into some challenges.
* Does not provide GUI to view the stack resources. GUI is available through paid licenses.
  i.e. [Commandeer](https://getcommandeer.com/)
...
```
## Using tflocal
To simplify the process of using Terraform with Localstack, you can use the `tflocal` tool. `tflocal` is a wrapper around Terraform that automatically configures the AWS provider to point to Localstack.
You can install `tflocal` using pip:
```bash
pip install tflocal
```
Once installed, you can use `tflocal` instead of `terraform` in your commands. For example, to apply the Terraform configuration, you would run:
```bash
tflocal apply
tflocal destroy
```
No need to set the AWS provider configuration manually, as `tflocal` handles that for you.  Do not need to configure endpoints.
```hcl
{
  endpoints {
